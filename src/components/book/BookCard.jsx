...
const BookCard = ({ book, cart, setCart }) => {
    ...
    const { image, tags, bookName, author, price, category, rating, bookId } = book;

    // Derive isAddedToCart directly from cart prop
    const isAddedToCart = cart.some(item => item.bookId === bookId);
    ...
    return (
        <div className='p-6 border border-green-500 shadow-md rounded-md space-y-3'>
            ...
            <button
                onClick={cartHandler}
                className={`btn ${isAddedToCart ? 'btn-success text-white' : 'btn-primary'} w-full`}

            >
                {isAddedToCart ? (
                    <span className='flex items-center'><TiTick />Added to Cart</span>
                ) : (
                    'Add To Cart'
                )}
            </button>
